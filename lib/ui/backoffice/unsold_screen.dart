...
/// Records how many units of each product remained unsold at the end of the day.
///
/// Design matches [ProductionScreen]:
///  • Lists every active product with a [–] / [+] counter.
///  • Top-right "Set date & time" button lets the manager backdate if logging
///    later.
...
class _UnsoldScreenState extends State<UnsoldScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await context.read<UnsoldRepository>().recordSession(lines);
      if (!mounted) return;
      ...
      showAppSnackBar(
        context,
        'Saved $totalQty unsold item${totalQty == 1 ? '' : 's'} · '
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Record Unsold'),
        actions: [
          Padding(
...
class _UnsoldRow extends StatelessWidget {
  const _UnsoldRow({
