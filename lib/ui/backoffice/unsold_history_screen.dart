...
/// One recording session containing one or more [UnsoldLog] entries.
class UnsoldSession {
  const UnsoldSession({
  ...
  static List<UnsoldSession> group(List<UnsoldLog> logs) {
    final map = <DateTime, List<UnsoldLog>>{};
    for (final log in logs) {
      map.putIfAbsent(log.recordedAt, () => []).add(log);
    }
    final sessions = map.entries
        .map((e) => UnsoldSession(recordedAt: e.key, lines: e.value))
        .toList();
...
/// Browse a day's unsold records.
///
/// Looks and behaves like [OrderHistoryScreen]: a date bar with back/forward
/// navigation at the top, and an expandable list of unsold sessions below.
class UnsoldHistoryScreen extends StatefulWidget {
  const UnsoldHistoryScreen({super.key});

  @override
  State<UnsoldHistoryScreen> createState() => _UnsoldHistoryScreenState();
}

class _UnsoldHistoryScreenState extends State<UnsoldHistoryScreen> {
  DateTime _date = startOfDay(DateTime.now());
  List<UnsoldSession> _sessions = const [];
  UnsoldDayTotals? _totals;
  bool _loading = true;
  String? _error;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Unsold history')),
      body: Column(
        children: [
          OrderDateBar(
            date: _date,
          ...
          if (!_loading && _error == null && totals != null && totals.totalQty > 0) ...[
            _UnsoldDaySummaryBar(totals: totals),
          ],
                ...
                : _error != null
                    ...
                    : _sessions.isEmpty
                        ? const EmptyView(
                            ...
                            message: 'No unsold products recorded on this day.',
                          )
                        : ListView.separated(
...
/// Compact bar summarizing the day's total unsold units and total value.
class _UnsoldDaySummaryBar extends StatelessWidget {
  const _UnsoldDaySummaryBar({required this.totals});

  final UnsoldDayTotals totals;
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(
            'Total: ${totals.totalQty} unsold unit${totals.totalQty == 1 ? '' : 's'} '
...
/// One unsold session tile, expanding to show product lines.
class _UnsoldSessionTile extends StatelessWidget {
  const _UnsoldSessionTile({required this.session});

  final UnsoldSession session;
